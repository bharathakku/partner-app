...
const STORAGE_KEY_PREFIX = 'partnerSubscription:'
...
export const getStoredSubscriptionForUser = (userId) => {
  if (typeof window === 'undefined') return null
  if (!userId) return null
  try {
    // Prefer user-scoped, fallback to legacy global key once (migration)
...
export const setStoredSubscriptionForUser = (userId, sub) => {
  if (typeof window === 'undefined') return
  if (!userId) return
  try { window.localStorage.setItem(`${STORAGE_KEY_PREFIX}${userId}`, JSON.stringify(sub)) } catch {}
}
...
export const clearStoredSubscriptionForUser = (userId) => {
  if (typeof window === 'undefined') return
  if (!userId) return
  try { window.localStorage.removeItem(`${STORAGE_KEY_PREFIX}${userId}`) } catch {}
}
