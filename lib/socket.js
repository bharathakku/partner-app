...
export function connectSocket() {
  ...
  socket = io(wsBase, { withCredentials: true, transports: ['websocket', 'polling'], reconnection: true })
  try {
    socket.on('connect', () => {
      console.log('[socket] connected', socket.id, '->', wsBase)
    })
...
export function joinDriverRoom(driverId) {
  ...
  try { console.log('[socket] join-driver', driverId) } catch {}
  socket.emit('join-driver', driverId)
}

export function leaveDriverRoom(driverId) {
  if (!socket || !driverId) return
  try { console.log('[socket] leave-driver', driverId) } catch {}
  socket.emit('leave-driver', driverId)
}

export function onOrderAssigned(handler) {
  if (!socket) return () => {}
  const events = ['order-assigned', 'order:assigned', 'assignment']
  try { console.log('[socket] listen:', events) } catch {}
